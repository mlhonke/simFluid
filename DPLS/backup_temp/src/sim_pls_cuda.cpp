...
//    cuda_check(cudaMemcpy(&pp[0], DEV_pp, n_pp*sizeof(CUVEC::Vec3d), cudaMemcpyDeviceToHost));
    ...
    if (iteration % reseed_interval == 0){
        propagate_interface_distances(100000*dx); // full grid propagation
    } else {
        propagate_interface_distances(100000*dx);
    }
...
void SimPLSCUDA::redistance_neighbour(const Vector3i &face, CubeX &unsigned_dist, const CubeXi &closest_point, std::queue<Vector3i> &cell_queue, int cp_id, scalar_t prop_bw){
...
//                }
    ...
    while (!cell_queue.empty()) {
        ...
        //Check neighbours;
        redistance_neighbour({i - 1, j, k}, LS_unsigned, cp, cell_queue, cp_id, prop_bw);
        redistance_neighbour({i + 1, j, k}, LS_unsigned, cp, cell_queue, cp_id, prop_bw);
        redistance_neighbour({i, j - 1, k}, LS_unsigned, cp, cell_queue, cp_id, prop_bw);
        redistance_neighbour({i, j + 1, k}, LS_unsigned, cp, cell_queue, cp_id, prop_bw);
        redistance_neighbour({i, j, k - 1}, LS_unsigned, cp, cell_queue, cp_id, prop_bw);
        redistance_neighbour({i, j, k + 1}, LS_unsigned, cp, cell_queue, cp_id, prop_bw);
