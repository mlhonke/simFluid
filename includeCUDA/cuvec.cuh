/* vec.h from Robert Bridson's common files.
 * https://www.cs.ubc.ca/~rbridson/download/common_2008_nov_12.tar.gz
 * Used here as a CUDA vector library since it is pure C and compiles well with nvcc.
 */
...
struct Vec
{
    ...
    __host__ __device__ Vec<N,T>(void)
    {}

    __host__ __device__ explicit Vec<N,T>(T value_for_all)
    { for(unsigned int i=0; i<N; ++i) v[i]=value_for_all; }
    
    template<class S>
    __host__ __device__ explicit Vec<N,T>(const S *source)
    { for(unsigned int i=0; i<N; ++i) v[i]=(T)source[i]; }
    
    template <class S>
    __host__ __device__ explicit Vec<N,T>(const Vec<N,S>& source)
    { for(unsigned int i=0; i<N; ++i) v[i]=(T)source[i]; }
    ...
    __host__ __device__ bool nonzero(void) const
    {
        for(unsigned int i=0; i<N; ++i) if(v[i]) return true;
        return false;
    }

    __host__ __device__ Vec<N,T> operator+=(const Vec<N,T> &w)
    {
        for(unsigned int i=0; i<N; ++i) v[i]+=w[i];
        return *this;
    }

    __host__ __device__ Vec<N,T> operator+(const Vec<N,T> &w) const
    {
        Vec<N,T> sum(*this);
        sum+=w;
        return sum;
    ...
    __host__ __device__ Vec<N,T> operator-(void) const // unary minus
    {
        Vec<N,T> negative;
    ...
    __host__ __device__ Vec<N,T> operator-(const Vec<N,T> &w) const // (binary) subtraction
    {
        Vec<N,T> diff(*this);
        diff-=w;
        return diff;
    ...
    __host__ __device__ Vec<N,T> operator*=(T a)
    {
        for(unsigned int i=0; i<N; ++i) v[i]*=a;
        return *this;
    }

    __host__ __device__ Vec<N,T> operator*(T a) const
    {
        Vec<N,T> w(*this);
        w*=a;
        return w;
...
template<unsigned int N, class T> __host__ __device__
inline Vec<N,T> clamp(const Vec<N,T> &a, const Vec<N,T> &min, const Vec<N,T> &max)
{
    Vec<N,T> l;
    for(unsigned int i=0; i<N; ++i){
        l.v[i] = fmax(a.v[i], min.v[i]);
        l.v[i] = fmin(l.v[i], max.v[i]);
    }
    return l;
}
