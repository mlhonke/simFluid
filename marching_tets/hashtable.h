...
struct HashTable {
    ...
    explicit HashTable(unsigned int expected_size = 64)
            : hash_function(HashFunction()), key_equal(KeyEqual()) { init(expected_size); }

    explicit HashTable(const HashFunction &hf, unsigned int expected_size = 64)
            : hash_function(hf), key_equal(KeyEqual()) { init(expected_size); }
    ...
    void output_statistics() const {
        ...
        std::cout << "rest: " << total - subtotal << "   (" << 100.0 * (1.0 - subtotal / (float) total) << "%)"
                  << std::endl;
